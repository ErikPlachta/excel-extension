# TypeScript Azure Function - Databricks SQL Middleware

Azure Functions TypeScript V4 template for connecting Excel add-in to Databricks.
...
```bash
cd examples/azure-functions/typescript
npm install
```
...
├── src/
│   ├── functions/
│   │   ├── executeQuery.ts   # POST /api/query/execute
│   │   └── getChunk.ts       # GET endpoints
│   └── services/
│       └── databricksClient.ts
├── package.json
├── tsconfig.json
...
2. **Connection reuse** is built-in via singleton `DatabricksClient`
3. **Native fetch** handles connection pooling automatically in Node.js 18+
...
The `DatabricksClient` uses a module-level singleton:

```typescript
// In functions
const client = getDatabricksClient(); // Reuses single instance
```

Node.js 18+ native fetch handles HTTP/1.1 keep-alive and HTTP/2 multiplexing automatically.

### Long-Running Queries

For queries >50s (Databricks sync limit):

1. `executeStatement()` returns 202 with `statementId`
